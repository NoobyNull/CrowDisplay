...
**IMPORTANT:** Do NOT recreate clock_screen or ota_screen in rebuild_ui or the helper -- those are separate LVGL screens created once in create_ui() and reused.
  ...
  <verify>
Run `cd /data/Elcrow-Display-hotkeys && pio run -e display` and confirm clean compilation. Verify `grep -n "lv_obj_clean\|lv_mem_monitor" display/ui.cpp` shows both functions present.
  </verify>
  <done>
