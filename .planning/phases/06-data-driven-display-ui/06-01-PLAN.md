...
**IMPORTANT:** Do NOT touch rebuild_ui() in this task -- that is handled in plan 06-02. Leave it as-is for now (it will still have the Hotkey bug, which plan 06-02 fixes when it rewrites rebuild_ui entirely).

Actually, since we are removing the Hotkey struct and button_config_to_hotkey(), rebuild_ui() will fail to compile. So we MUST also update rebuild_ui() in this task to use ButtonConfig* instead of Hotkey. Update the rebuild_ui() page creation loop to match the create_ui() pattern:
  ```cpp
  for (size_t j = 0; j < page.buttons.size(); j++) {
      create_hotkey_button(tab, &page.buttons[j]);
  }
